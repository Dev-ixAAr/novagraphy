...
// --- STATIC DATA (not managed via admin) ---
const TIMELINE = [
...
function FounderSection({ founder }: { founder: TeamMemberData }) {
    return (
                ...
                <div className="flex flex-col md:flex-row items-center gap-12 md:gap-24">
                    <div className="w-full md:w-1/2 relative group">
                        <div className="relative aspect-[4/5] overflow-hidden rounded-2xl border border-foreground/10 shadow-2xl">
                            <Image src={sanitizeImageUrl(founder.img)} alt={founder.name} fill className="object-cover grayscale group-hover:grayscale-0 transition-all duration-1000 ease-out" />
                            <div className="absolute bottom-8 right-8 mix-blend-difference"><span className="font-gudlak text-6xl text-white opacity-80">{founder.name.split(" ")[0]}.</span></div>
                        </div>
                    </div>
...
function TeamGrid({ team }: { team: TeamMemberData[] }) {
    return (
        <section className="py-24 px-6 md:px-12 border-t border-foreground/5">
            <div className="max-w-7xl mx-auto">
                <div className="mb-16 flex justify-between items-end">
                    ...
                    <span className="font-share-tech text-xs text-electric-blue border border-electric-blue/30 px-3 py-1 rounded-full">
                ...
                {team.length > 0 ? (
                        ...
                        {team.map((member) => (
                            <div key={member.id} className="group relative cursor-pointer">
                                <div className="relative aspect-[3/4] w-full overflow-hidden rounded-xl bg-background mb-6 border border-foreground/5 shadow-lg dark:shadow-none">
                                    <Image src={sanitizeImageUrl(member.img)} alt={member.name} fill className="object-cover transition-all duration-700 ease-out grayscale group-hover:grayscale-0 group-hover:scale-110 z-10" />
                                </div>
                    ...
                    <p className="text-center text-foreground/40 font-base-neue py-12">No team members to display yet.</p>
                )}
            </div>
        </section>
    );
