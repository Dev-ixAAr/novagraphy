...
interface ProductWithRelations {
  ...
  status: string | null;
  featured: boolean;
  colors: { id: string; name: string; hex: string | null; images: string[] }[];
  sizes: { id: string; label: string; priceMod: number }[];
}
...
export default function EditProductForm({ product }: EditProductFormProps) {
  // Main image state — pre-filled
  const [imageUrl, setImageUrl] = useState(product.image);

  // --- Colors / Variants State — pre-filled from DB ---
  ...
  // --- Sizes State — pre-filled from DB ---
  ...
  const updateColor = (
    index: number,
    field: keyof Omit<ColorVariant, 'images'>,
    value: string
  ) => {
    setColors((prev) =>
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          ...
          <p className="text-sm text-gray-400">
            Update details for{' '}
            <span className="text-indigo-400">{product.title}</span>
          </p>
        </div>
      </div>
          ...
          <div className="bg-[#161920] border border-gray-800 rounded-xl p-6 space-y-5">
            ...
            {colors.length === 0 && (
              <p className="text-sm text-gray-500 italic text-center py-4">
                No color variants added yet. Click &quot;Add Color&quot; to
                begin.
              </p>
            )}
                ...
                <div className="space-y-3">
                  ...
                  {color.images.length === 0 && (
                    <p className="text-xs text-gray-600 italic">
                      No images added for this color.
                    </p>
                  )}
                  ...
                  {color.images.map((img, ii) => (
                    <div key={ii} className="flex gap-2 items-center">
                      <input
                        ...
                        value={img}
                        onChange={(e) =>
                          updateImageInColor(ci, ii, e.target.value)
                        }
              ...
              >
                ...
                <div className="flex-1 w-full space-y-1.5">
                  ...
                  <input
                    ...
                    value={size.priceMod}
                    onChange={(e) =>
                      updateSize(si, 'priceMod', parseFloat(e.target.value) || 0)
                    }
                    className={inputClass}
                  />
                </div>
