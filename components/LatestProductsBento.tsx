    ...
    <section className="w-full bg-background py-24 px-6 md:px-12 border-t border-white/5">
      <div className="max-w-7xl mx-auto">
        ...
        {featuredProducts.length === 0 ? (
          <div className="w-full py-24 flex flex-col items-center justify-center border border-white/10 border-dashed rounded-2xl bg-white/5">
            <ShoppingBag className="w-12 h-12 text-gray-600 mb-4" />
            <p className="font-share-tech text-gray-400 tracking-widest uppercase">No products dropped yet.</p>
          </div>
        ) : (
          <div className="flex overflow-x-auto snap-x snap-mandatory [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] gap-4 md:grid md:grid-cols-4 md:grid-rows-2 md:h-[600px] w-full pb-4 md:pb-0">
              ...
              return (
                <motion.div
                  ...
                  className={`
                    ...
                    shrink-0 w-[80vw] h-[400px] md:w-auto md:h-auto snap-center
                    ${gridClass}
                  `}
                  whileHover={{ y: -5 }}
                >
                  ...
                  <div className="absolute inset-0 w-full h-full">
                    <Image
                      src={product.colors?.[0]?.images?.[0] || product.image || "/placeholder.jpg"}
                      alt={product.title}
                      fill
